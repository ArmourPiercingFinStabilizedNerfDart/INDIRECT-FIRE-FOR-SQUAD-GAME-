...
vehicles = {
    ...
    "INF": {
        "QLZ87": {
            ...
            "offset": {"x": 0, "y": 0},
            "projectiles": {
                "QLZ87GL": {"velocity": 190, "gravity_modifier": 1},
            },
        },
    },
}
...
def calculate_screen_offset(v, g, d, dy, resolutionh, resolutionv, FOVh_degrees, aspect_ratio):
    ...
    if inside_sqrt < 0:
        return None, "No ballistic solution possible"
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ballistic calculation")
        ...
        self.toggleCrosshairButton = QPushButton("Activate crosshair")
        ...
        self.resolutionHInput.setValue(1920)
        self.resolutionVInput.setRange(1, 10000)
        self.resolutionVInput.setValue(1080)
        ...
        heightLayout.addWidget(QLabel("Height difference (m):"))
        ...
        resolutionLayout.addWidget(QLabel("Horizontal resolution:"))
        resolutionLayout.addWidget(self.resolutionHInput)
        resolutionLayout.addWidget(QLabel("Vertical resolution:"))
        resolutionLayout.addWidget(self.resolutionVInput)
        layout.addLayout(resolutionLayout)

        aspectLayout = QHBoxLayout()
        aspectLayout.addWidget(QLabel("Aspect ratio (personalized):"))
    ...
    def projectileChanged(self, index):
        ...
        if decalage_pixels is None:
            ...
            result_text = (f"Shooting angle: {angle_degrees:.2f}°\n"
                           f"Flight time: {flight_time:.2f} s\n"
                           f"Vertical offset in pixels: {decalage_pixels:.2f}")
    ...
    def toggleCrosshair(self, checked):
        if checked:
            self.toggleCrosshairButton.setText("Disable crosshair")
            self.projectileChanged(self.projectileComboBox.currentIndex())
        else:
            self.toggleCrosshairButton.setText("Activate crosshair")
            if self.crosshair:
                self.crosshair.close()
                self.crosshair = None
